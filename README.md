...
- PLC_1 (192.168.0.1): Station‑1 “Automation Production Station”
    - Core operations such as RPP (rotary pick-and-place), gripper open/close, dispensing extend/retract, rotation CW/CCW, homing
    - Blocks: AUTO 1 [FC1], OUTPUT CALL 1 [FC2], ESTOP LOGIC 1 [FC3], DATA BLOCK MOVE [FC4], COM 1 [FC5], Main [OB1]
    - Inter-station DBs: STATION-1 INPUT AND OUTPUT DATA [DB12], STATION 2 INPUT OUTPUT DATA [DB13], STATION‑3,4,5 INPUT [DB18]
    - Uses S7 GET/PUT via GET_DB/PUT_DB instances
    - I/O example: DI14/DQ10 onboard, AI2(0–10V) at IW64/IW66
- PLC_2 (192.168.0.2): Station‑2 “Handling/Conveyor/Inspection”
    - Clamping and handling cylinder motion (extend/down/up/retract), conveyor run/stop, metal detection and rejection sequence, height check with AI
    - Blocks: AUTO 2 [FC1], OUTPUT CALL 2 [FC2], ESTOP LOGIC 2 [FC3], DATA BLOCK MOVE [FC4], COM 2 [FC5], Main [OB1]
    - Inter-station DBs: STATION 1 INPUT OUTPUT [DB15], STATION 3 INPUT OUTPUT [DB16], STATION 2 INPUT OUTPUT [DB17]
    - Analog height check: IW64 converted to REAL (%MD70) and compared against threshold
    - GET/PUT with dedicated DBs
- PLC_3 (future/aux station scaffold)
    - Communication scaffolding (DATA BLOCK MOVE, COM 3, Main)
    - Tag/DB structure prepared for integration (e.g., with Station‑1/2)
...
- TIA Portal V16 project with:
    - PLCs: PLC_1, PLC_2, PLC_3 (and scaffold for PLC_4)
    - Program blocks (FC/OB), system IEC timer DBs
    - Data Blocks for station IO and inter-station comms
    - PLC tag tables and default tag sets
    - Traces/measurements scaffolds included for future diagnostics
...
3. Set IP addresses:
    ...
    - Adjust additional PLCs as needed
4. Enable PUT/GET communication and confirm the same in Protection \& Security.
5. Download the hardware config and software to each PLC.
6. Use the Station DBs (DB12/DB13/DB15/DB16/DB17/DB18, etc.) for HMI bindings and manual testing.
7. In Online mode, monitor AUTO n, OUTPUT CALL n, and ESTOP LOGIC n networks to validate sequences.
